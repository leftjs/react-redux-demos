...
class App extends Component {
	render() {
		...
		return (
			<div>
				<AddTodo onAddSubmit={text => dispatch(addTodo(text))}></AddTodo>
				<TodoList
				todos={visibleTodos}
...
function seleteTodos(todos, filter) {
	switch (filter) {
...
function select(state) {
	return {
		...
		visibleTodos: seleteTodos(state.todos.present, state.visibilityFilter),
